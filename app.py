...
def api_check():
    ...
    prediction_label = "phishing" if phishing_score >= 50 else "safe"
...
def debug_features():
    ...
    response = {
        ...
        "model_columns_present": model_columns is not None,
    }
    ...
    if model:
        try:
            ...
            if model_columns:
                ...
                X_df = X_df[model_columns]
            proba = model.predict_proba(X_df)[0]
            response["predict_proba"] = proba.tolist()
            response["classes"] = model.classes_.tolist()
            response["probability_class1"] = float(proba[-1])
            response["blended_percent"] = round((float(proba[-1]) * 0.7 + homoglyph_score * 0.3), 2)
        except Exception as e:
...
def block_url():
    ...
    db = get_db()
    db.execute(
        "INSERT INTO blocked (url, reason, ts) VALUES (?, ?, ?)",
        (url, reason, datetime.datetime.utcnow().isoformat()),
    )
    db.commit()

    return jsonify({"status": "blocked", "url": url, "reason": reason})
